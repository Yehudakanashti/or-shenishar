...
5א. אם למוצר רשום ״אסור להזכיר״ — אל תזכיר את זה בשום צורה, בפומבי ובפרטי.
    זו הוראה קשיחה, לא המלצה.
6. {address_form}

## מטרת התגובה הציבורית
המטרה היחידה שלה היא לפתוח שיחה בפרטי. לא לסגור עסקה, לא לפרט, לא לשכנע.
משפט שמראה שיש לנו מענה, ומשם ממשיכים בפרטי. מחיר, אספקה ופרטי הזמנה —
כולם נסגרים בפרטי, אף פעם לא בתגובה הפומבית.
...
def _catalog_block(products: list[dict[str, Any]]) -> str:
    ...
    for p in products:
        ...
        if p.get("avoid"):
            lines.append(f"אסור להזכיר: {p['avoid']}")
        if p.get("notes"):
